...
private bool altKeyReleased = true;  // A flag to track if Alt was released
    ...
    {   
            PlayerPrefs.SetInt("prefcard", 0);
                        PlayerPrefs.SetInt("ismovedcard", 0);
                        PlayerPrefs.SetInt("ismovedusb", 0);
...
#region Camera
    ...
    {
        lockCursor = false;  // Unlock the cursor when Alt is pressed
        altKeyReleased = false;  // Reset the flag as Alt is now being pressed
    }

    // Detect Alt key release and handle it only once
    if (!altKeyReleased && (Input.GetKeyUp(KeyCode.LeftAlt) || Input.GetKeyUp(KeyCode.RightAlt)))
    {
        lockCursor = true;  // Lock the cursor when Alt is released
        altKeyReleased = true;  // Mark that Alt has been released, so this is only handled once
    }
// Detect if both Alt keys are released
...
if (cameraCanMove)
{
    ...
    float mousePitch = Input.GetAxis("Mouse Y") * mouseSensitivity;

    // Get joystick input for yaw and pitch

    // Calculate total yaw
    yaw = transform.localEulerAngles.y + mouseYaw ;

    // Calculate pitch based on inverted setting
    if (!invertCamera)
    {
        pitch -= mousePitch ; // Normal Y
    }
    else
    {
        // Inverted Y
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        GUILayout.Label("By Mohamed dhia werteteni", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Normal, fontSize = 12 });
